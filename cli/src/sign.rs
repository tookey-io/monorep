...
async fn main() -> Result<()> {
    ...
    let (i, incoming, outgoing) =
        join_computation::<serde_json::Value>(args.address.clone(), &args.room)
            .await
    ...
    if i != expected_id {
        return Err(anyhow!(
            "Incorrect party id. Voting for group is not implemented yet"
        ));
    }
        ...
        match msg {
            Ok(msg) => {
                ...
                let possible_offline =
                    serde_json::from_value::<OfflineProtocolMessage>(json.clone());
                ...
                let wrapped_msg = match (possible_offline, possible_partial) {
                    ...
                    (_, Ok(partial)) => {
                        ecdsa::Messages::PartialSignature(msg.map_body(|_| partial))
                    }
                    _ => unreachable!(),
                };
    ...
    let outgoing_sender = futures::sink::unfold(outgoing, |mut outgoing, msg| async move {
        let packet = match msg {
            ecdsa::Messages::OfflineStage(msg) => msg.map_body(|body| {
                serde_json::to_value(body)
                    .context("packet serialization")
                    .unwrap()
            }),
            ecdsa::Messages::PartialSignature(msg) => msg.map_body(|body| {
                serde_json::to_value(body)
                    .context("packet serialization")
                    .unwrap()
            }),
    ...
    let signature = tss_ceremonies::ecdsa::sign(
        outgoing_sender,
        incoming,
        local_share,
        args.parties,
